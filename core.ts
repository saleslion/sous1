...
export function createExpandableRecipeCard(
    ...
    // Add recipe source indicator
    const sourceInfo = mainRecipeData.source === 'themealdb' ? 
        '<span class="recipe-source" title="Recipe from TheMealDB community database">🌍 Traditional Recipe</span>' :
        '<span class="recipe-source" title="Recipe generated by Sousie AI">🤖 AI Created</span>';
    
    let summaryHTML = `<h3>${sanitizeHTML(overallName)} ${sourceInfo}</h3>
    ...
    try {
        const { error } = await supabaseClient
            ...
            .upsert({
                ...
                source: recipeData.source || 'ai_generated'
